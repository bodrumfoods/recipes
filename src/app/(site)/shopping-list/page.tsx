  ...
  return (
    <div className="mx-auto w-full max-w-3xl px-4 py-10 sm:px-6">
      <div className="animate-fade-in-up">
        <h1 className="font-display text-3xl font-bold text-brand-ink">Shopping List</h1>
      ...
      <section className="mt-8 animate-fade-in-up">
        ...
        <ul className="divide-y divide-brand-border rounded-2xl border border-brand-border bg-brand-card">
          {matched.map((item) => (
            <li
              key={item.key}
            ...
            >
              <div>
                <p className="font-medium capitalize text-brand-ink">{item.name}</p>
                <p className="text-xs text-brand-ink/50">{item.productTitle}</p>
              </div>
              <div className="flex items-center gap-3">
                ...
                {!item.variantId && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="whitespace-nowrap rounded-full border border-brand-border px-3 py-1 text-xs font-medium text-brand-ink hover:border-brand-green"
                  >
                    View Product
                  </a>
                )}
          ...
          {matched.length === 0 && (
            <li className="px-4 py-3 text-sm text-brand-ink/50">No matching ingredients.</li>
          )}
        </ul>
      </section>
          ...
          <ul className="divide-y divide-brand-border rounded-2xl border border-brand-border bg-brand-card">
            {unmatched.map((item) => (
              <li
                ...
                className="flex items-center justify-between gap-3 px-4 py-3 text-sm"
              >
                <span className="capitalize text-brand-ink">{item.name}</span>
                <span className="font-medium text-brand-ink">
                  {item.amount} {item.unit}
                </span>
              </li>
